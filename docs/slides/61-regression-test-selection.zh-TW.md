...
- 動態切片只包含值確實在那次特定執行中流向輸出的陳述式——而非任何在理論上可能如此的陳述式。
...
|---|---|---|---|
| `pos` | s2, s4, s5, s11 | 否 | **可安全略過** |
| `neg` | s2, s4, s7, s8, s11 | 否 | **可安全略過** |
| `zero` | s2, s4, s7, s11 | 否 | **可安全略過** |
...
| 屬性 | 靜態 | 動態 |
|---|---|---|
| 影響集合 | 前向切片 `FS(s)`——結構性 | 前向切片 `FS(s)`——結構性 |
