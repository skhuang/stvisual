...
paginate: true
size: 16:9
title: 軟體測試視覺化 #59 — 缺陷定位與切塊
description: 利用程式切塊從失敗切片縮小到可疑陳述式集合 —— 靜態多輸出切塊與動態多輸入切塊。
lang: zh-TW
---
...
軟體測試視覺化系列 #59 · 切片式測試
搭配工具：`/section-slicing` → 切塊分頁（[SliceDicingExplorer](../../src/components/SliceDicingExplorer.js)）

<!-- 直接建立在第 58 講（程式切片）之上。核心轉變：切片告訴你「這些陳述式『可能』造成了錯誤」；切塊告訴你「這些是讓錯誤輸出與正確輸出產生差異的陳述式」。 -->
...
<!-- Weiser 1984 年的原始切片論文就已承認這一點：切片縮小了搜索範圍，但仍可能很大。Lyle 與 Weiser 在 1987 年的後續論文引入了切塊，利用正確輸出提供的額外資訊。 -->

---

## 程式切塊 —— 核心定義

錯誤輸出 `W`、正確輸出 `C₁, C₂, …` 的**程式切塊**：
...
- 剩下的就是**切塊**：唯一影響錯誤輸出、且未與任何正確輸出共享的陳述式。

切塊中的陳述式負責計算正確輸出中所沒有的東西 —— 這是缺陷位置的強力指示。

<!-- 集合差是關鍵運算。C₁… 的正確性告訴我們，它們切片中的陳述式「運作正確」（至少對此次執行而言）。減去它們，就隔離出了唯一出錯的計算。 -->

---

## 靜態多輸出切塊 —— Lyle–Weiser 方法
...
**無辜聯集** = slice(`total`) ∪ slice(`mean`) = {s2, s4, s5, s10, out-total, out-mean}

**dice(`highest`)** = {s3, s4, s6, s7, out-highest} − {s2, s4, s5, s10, out-total, out-mean}
= **{s3, s6, s7, out-highest}**

陳述式 `s7`（`highest = total`）在切塊中 —— 正是錯誤所在。

<!-- 注意 s4（for 迴圈標頭）從切塊中消失了，因為它同時出現在 total 和 mean 的切片中 —— 迴圈結構是共享的。留下來的是 highest 的初始化（s3）、條件判斷（s6）、更新（s7）以及輸出本身。錯誤就在 s7。 -->

---

## 為什麼靜態切塊在不同輸入間無效

假設我們嘗試用靜態切片進行多*輸入*切塊：

- 失敗執行：`fare(30, true)` → 實際 14，預期 12。
- 通過執行：`fare(30, false)` → 實際 10，預期 10。
- `return price` 處 `price` 的靜態向後切片對兩次執行都**相同** —— 靜態切片是**與輸入無關的**。

$$\text{靜態切塊} = \text{staticSlice}(\text{失敗}) \setminus \text{staticSlice}(\text{通過}) = \emptyset$$

**當準則是相同的陳述式與變數時，靜態切塊永遠是空集合** —— 減去相等的集合什麼也不剩。

這就是**動態切片**對於跨不同測試輸入的缺陷定位至關重要的原因。

<!-- 這是本規格強調的核心洞見。靜態切片過度近似：它包含了在任何輸入下*可能*影響輸出的每個陳述式。同一程式的兩次不同執行，對相同準則產生相同的靜態切片。切塊需要切片之間的差異 —— 而這種差異只出現在動態切片中。 -->
...
<!-- 此方法由 Agrawal、DeMillo 與 Spafford（《Debugging with Dynamic Slicing and Backtracking》，1993）推廣，並由 Korel 與 Laski 進一步形式化。相對於靜態切塊，其核心優勢在於不同輸入確實會對相同準則產生不同的動態切片。 -->
...
| B | age=30, peak=false | 10 | 10 | 通過 | {s2, s8, s11} |
| C | age=12, peak=false | 5 | 5 | 通過 | {s2, s3, s4, s8, s11} |
| D | age=70, peak=false | 3 | 3 | 通過 | {s2, s3, s5, s6, s8, s11} |

<!-- 三條通過軌跡共同涵蓋了每個年齡分支。關鍵是：因為所有通過執行中 peak=false，所以沒有任何一條通過軌跡走到 s9。 -->
...
**通過聯集**（軌跡 B ∪ C ∪ D）：{s2, s3, s4, s5, s6, s8, s11}

**切塊** = {s2, s8, s9, s11} − {s2, s3, s4, s5, s6, s8, s11} = **{s9}**

切塊只包含一個陳述式：`s9`（`price = price + 2 + 2`）—— 正是有錯誤的那行。

注意：s2、s8、s11 出現在通過軌跡中，因此被減掉。年齡分支（s3–s6）出現在通過軌跡中，因此也被減掉。只有 s9 —— 尖峰附加費，僅由失敗軌跡走到 —— 留了下來。

<!-- 這就是動態切塊的威力：通過軌跡充當「差分測試」，直接指出造成失敗的那個陳述式。在實務中，越多樣化的通過軌跡能給出越小、越精確的切塊。 -->

---

## 工具演示

在 `/section-slicing` 開啟**切塊**分頁（切塊探索器）：

1. **靜態模式** —— 選取 `summaryStats` 場景。
   - 在 PDG 中看到三個輸出變數的切片高亮顯示。
   - `highest`（錯誤輸出）的切塊以最強的高亮顯示 —— 觀察 `s7` 被隔離出來。
2. **動態模式** —— 選取 `fare` 場景。
   - 四條軌跡都列出，並附有執行結果標籤。
   - 失敗軌跡的動態切片與通過聯集以兩種色調顯示。
   - 切塊（{s9}）是唯一以最強高亮顯示的陳述式。
3. 注意詳細面板：切塊大小、確認錯誤陳述式位於切塊之中。
4. 嘗試小測驗：「切塊指向哪個陳述式？」
...
- **程式切塊**（Lyle & Weiser，1987）從錯誤輸出的切片中減去*正確*輸出的切片，只留下唯一影響缺陷的陳述式。
- **靜態多輸出切塊**利用同一次執行中的不同輸出變數；其有效性在於不同輸出變數具有真正不同的靜態切片。
- **跨不同輸入的靜態切塊永遠是空集合** —— 靜態切片與輸入無關，集合差運算什麼也不剩。
- **動態多輸入切塊**使用特定執行的動態切片；通過軌跡減去無辜的路徑，只留下失敗所獨有的部分。

**課堂練習：** 用手追蹤 `fare` 全部四個輸入的動態切片，再在探索器中驗證切塊。

---

## 延伸閱讀

- 課程規格 —— 切塊設計（[2026-05-18-slicing-n2-dicing-design.md](../superpowers/specs/2026-05-18-slicing-n2-dicing-design.md)）
